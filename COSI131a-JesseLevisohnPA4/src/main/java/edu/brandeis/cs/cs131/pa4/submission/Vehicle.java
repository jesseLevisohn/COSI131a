 ...
 * 
 * I edited the doWhileInTunnel method to allow the vehicles to provide for the separate implementation of 
...
public abstract class Vehicle extends Thread implements Comparable<Vehicle>{
    ...
    public final void doWhileInTunnel() {
    	int timeLeft = (10-speed) * 100;
    	ReentrantLock lock = ((PreemptivePriorityScheduler) this.scheduler).lock;
    	Condition cv = ((PreemptivePriorityScheduler) this.scheduler).cv;
    	int i = 0;
    	...
    	while (timeLeft > 0) {
    		...
    		try {
        		// Initially, timeLeft is (10-speed) * 100
        		sleep(timeLeft);
        		
    			// If the vehicle is an ambulance, it should signal all the other vehicles when it is done in the tunnel
        		if (priority == 4) {
        			cv.signalAll();
    			}
    			...
    			try {
    				...
    				// timeLeft is calculated based on how much time has passed
    				// The time is calculated immediately after it is interrupted so that it does not include 
    				// the time spent waiting in the distance it has traveled.
    				int curr = (int) System.currentTimeMillis();
    				int timePassed = curr - start;
